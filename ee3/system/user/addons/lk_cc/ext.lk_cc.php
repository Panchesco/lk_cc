...
class Lk_cc_ext {
	...
	public function __construct($settings = array())
	{
		 ...
		 * ================================================ */
		// first check config
		if (ee()->config->item('lk_cc_cc_username') && ee()->config->item('lk_cc_cc_password') && ee()->config->item('lk_cc_cc_api_key'))
		{
			$this->config_loc = 'config';
		}
		// check in global vars
		elseif (array_key_exists('lk_cc_cc_username', ee()->config->_global_vars) && array_key_exists('lk_cc_cc_password', ee()->config->_global_vars) && array_key_exists('lk_cc_cc_api_key', ee()->config->_global_vars))
		{
	...
	public function get_settings()
	{
		...
		if (isset(ee()->session->cache['lk_cc']['settings']))
		{
			$this->settings = ee()->session->cache['lk_cc']['settings'];
			return;
		}
		...
		switch ($this->config_loc) :
			...
			case ('config') :
				$this->settings['cc_api_key'] = ee()->config->item('lk_cc_cc_api_key');
				$this->settings['cc_username'] = ee()->config->item('lk_cc_cc_username');
				$this->settings['cc_password'] = ee()->config->item('lk_cc_cc_password');
				if (isset(ee()->TMPL)) {
					ee()->TMPL->log_item('LK CC has retrieved settings from config.');
				}
			break;
			
			case ('global') :
				$this->settings['cc_api_key'] = ee()->config->_global_vars['lk_cc_cc_api_key'];
				$this->settings['cc_username'] = ee()->config->_global_vars['lk_cc_cc_username'];
				$this->settings['cc_password'] = ee()->config->_global_vars['lk_cc_cc_password'];
				if (isset(ee()->TMPL)) {
					ee()->TMPL->log_item('LK CC has retrieved settings from global variables.');
				}
			break;
			...
			default :
				...
				if ($query->num_rows() > 0)
				{
					...
					if (isset(ee()->TMPL)) {
						ee()->TMPL->log_item('LK CC has retrieved settings from DB.');
					}
				}
				else
				{
					if (isset(ee()->TMPL)) {
						ee()->TMPL->log_item('LK CC has not yet been configured.');
					}
				}
			break;
		...
		ee()->session->cache['lk_cc'] = array(
			'settings' => array(),
			'js' => array(),
			'css' => array()
		);
		ee()->session->cache['lk_cc']['settings'] = $this->settings;
