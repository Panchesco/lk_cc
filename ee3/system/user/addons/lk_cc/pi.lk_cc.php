...
class Lk_cc {
	...
	public function __construct()
	{
		...
		// grab settings
		$this->cc_api_key = ee()->session->cache['lk_cc']['settings']['cc_api_key'];
		$this->cc_username = ee()->session->cache['lk_cc']['settings']['cc_username'];
		$this->cc_password = ee()->session->cache['lk_cc']['settings']['cc_password'];
    ...
    public function subscribe() {
		ee()->lang->load('lk_cc', '', FALSE, TRUE, PATH_THIRD.'lk_cc/');
		$tagdata = ee()->TMPL->tagdata;
		$default = array();
		$show = false;
		
		if (isset(ee()->TMPL->tagparams['default_list'])) {
			$default = explode("|",ee()->TMPL->tagparams['default_list']);
			foreach ($default as $k => $v) {
				$default[$k] = trim($v);
			}
		}

        if (isset(ee()->TMPL->tagparams['show_list'])) {
			$show = explode("|",ee()->TMPL->tagparams['show_list']);
			foreach ($show as $k => $v) {
				$show[$k] = trim($v);
			}
		}
		...
		if ($variables[0]['email'] != "") {
			...
			if (count($subscribe) == 0) {
				$variables[0]['error_message'] = ee()->lang->line('pick_list');
				$variables[0]['email'] = '';
			} else {
				...
				if ($ccContactOBJ->subscriberExists(urlencode($variables[0]['email'])) === false) {
					...
					if (!$ccContactOBJ->addSubscriber($contactXML)) {
						$variables[0]['error_message'] = ee()->lang->line('unknown_error');
					} else {
						$success = true;
					}
				} else {
	...
	private function _flightcheck()
	{
		...
		switch ('flightcheck') :
			...
			default :
				ee()->TMPL->log_item('LK CC has passed flightcheck.');
			break;
	...
	*/
    ...
    public function campaign() {
    	...
    	if ( empty( ee()->session->cache[ 'Lk_cc' ][ 'shutdown_is_registered' ] ) )
		{
			ee()->session->cache[ 'Lk_cc' ][ 'shutdown_is_registered' ] = TRUE;
